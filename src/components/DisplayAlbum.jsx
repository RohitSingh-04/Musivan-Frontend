...
const DisplayAlbum = () => {
  ...
  return (
    <>
        <Navbar/>
        <DisplayInfo TopImage={(PlaylistView.length > 0) ? PlaylistView[0].name.image : null} Type="Playlist" Name={(PlaylistView.length > 0) ? PlaylistView[0].name.name : ""} Desc={(PlaylistView.length > 0) ? PlaylistView[0].name.desc : ""} Likes = {(PlaylistView.length > 0) ? PlaylistView[0].name.likes : 0} Songs = {PlaylistView.length} ObjectId={id}/>
